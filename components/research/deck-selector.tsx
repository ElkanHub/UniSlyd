...
export function DeckSelector({
    ...
    const selectedDecks = availableDecks.filter(d =>
        selectedIds.includes(d.id)
    )

    return (
        <div className="flex flex-col gap-2 w-full max-w-full overflow-hidden">
            {/* TOP BAR ROW — HARD CLAMPED */}
            <div className="flex items-center gap-2 w-full max-w-full overflow-hidden">

                {/* SCROLLABLE BADGES ONLY */}
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 overflow-x-auto whitespace-nowrap px-1 scrollbar-none">
                        {selectedDecks.map(deck => (
                            <Badge
                                key={deck.id}
                                variant="secondary"
                                className="pl-2 pr-1 h-7 text-xs flex items-center gap-1 shrink-0 max-w-[120px] md:max-w-[200px]"
                            >
                ...
                {/* FIXED SOURCES BUTTON */}
                <Dialog open={isOpen} onOpenChange={setIsOpen}>
                    <DialogTrigger asChild>
                        <Button
                            variant="outline"
                            size="sm"
                            className="h-7 text-xs border-dashed gap-1 shrink-0 bg-background/50 backdrop-blur-sm shadow-sm"
                        >
                            ...
                            <div className="flex flex-col gap-2">
                                {availableDecks.length === 0 ? (
                                    <p className="text-sm text-muted-foreground text-center py-4">
                                        No decks available. Upload some first.
                                    </p>
                                ) : (
